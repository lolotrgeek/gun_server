	...
	ctx.on('put', function(at){
		...
		if( skip_put && skip_put == at[ACK_] ) {
			if( _debug ) {
				...
				_debug_counter++;
				console.log( new Date(), "skipping put in-get:", _debug_counter, " get putting:", skip_put, at[ACK_], JSON.stringify( at._.put ) ); 
			}
			return;
		}
		_debug && console.log( new Date(), "PUT", at[SEQ_], at[ACK_], JSON.stringify( at._.put ) );
		Gun.graph.is(at._.put, null, function(value, field, node, soul){ var id;
...
// SERVER PROCESS HERE
	...
	const gun = Gun({
		...
		, db: {
			file: "gun.db",
			exclusive: false //If you want to have maximum speed, you can set exclusive, 
			// which will gain about 30-40% speed; but you're only allowed one instance of Gun against this database. 
